...
class Displacer(object):
    '''
    Displaces a point location while preserving its location inside a given polygon
    '''
...
class GeometryHelpers(object):
    ...
    # Convert meters to deciman degrees
    ...
    def relatePointsToPolygons(self, point_feature_class, polygon_feature_class):
        '''Input a point and polygon feature class and receive a dictionary of which points are in which polygons'''
...
if __name__ == '__main__':
    try:
        ...
        #Setup Basic Report
        REPORT_NAME = '_'.join(['Point_Displacement_Report', oGeometryHelpers.createTimestamp() + '.txt'])
        ...
        polygon_fields = arcpy.GetCount_management(POINTS_PATH).getOutput(0)
        ...
        assert arcpy.Exists(POINTS_PATH), 'Point Feature Class does not appear to exist'
        assert arcpy.Exists(POLYGON_PATH), 'Polygon Feature Class does not appear to exist'
        ...
        '''
        Many of the point datasets imported from the excel contain Lat/Long fields which are static attributes. 
        We want these fields to be updated to the new lat/long after the point is displaced. 
        The script writes to these fields later on in the 'while' loop, but the code immediately below is used to make sure the fields supplied in the tool dialog actually exist
        '''
        ...
        if(URBAN_RURAL_MODE):
            assert (URBAN_RURAL_FIELD in point_field_dict.keys()), URBAN_RURAL_FIELD + ' no in point feature class fields'
            point_search = arcpy.SearchCursor(POINTS_PATH)
            for p in point_search:
                locality = p.getValue(URBAN_RURAL_FIELD)
                assert (locality == URBAN_VALUE or locality == RURAL_VALUE), str (locality) + ' does not match urban/rural values provided. Please check your attribute table and make sure all urban/rural values match what was entered in the tool dialog.'
            del point_search
        ...
        #Loop throuhg each of the points
        for p in point_rows:
            point_geom = p.getValue(point_shapefield)

            #Get Max Distance based on tool mode and current point attirbutes (urban vs. rural)
            ...
            for q in polygon_rows:
                ...
                if(point_geom.within(poly_geom)):
                    ...
                    while(not new_point_within):
                        ...
                        displaced_x, displaced_y = oDisplacer.displacePoint(meter_x, meter_y, max_displace_distance)
                        ...
                        #Check if this point still remains inside the original polygon, if so
                ...
                arcpy.SetProgressorLabel('Displaced ' +str(displaced_points) + ' of ' + str(point_count) + mode_name)
                arcpy.SetProgressorPosition()

            #Handle if point is not found within any of the study area polygons

            if(not point_within_study_area):
                arcpy.AddWarning('Point Detected outside of study area. Please check report for more details.')
                report.write('POINT OUTSIDE STUDY AREAS: \n')
        ...
        tb = sys.exc_info()[2]
        msg = "An error occured on line %i" % tb.tb_lineno
        report.write('Exception: %s' %e + '/n')
        report.write(msg + '\n')
